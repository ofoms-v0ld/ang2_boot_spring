...
function include_async_code()
{
    ...
    fileref.setAttribute("src", "app/crypto_pro/sources/async_code.js");
...
function SignCadesBES_parallelRaz(certListBoxId) {
    var canAsync = !!cadesplugin.CreateObjectAsync;
    if (canAsync) {
        include_async_code().then(function () {
            return Sign_parallelRaz(certListBoxId);
                    ...
                    if (isPluginWorked) { // плагин работает, объекты создаются
                        ...
                        if (isPluginLoaded) { // плагин загружен
                            if (!isPluginEnabled) { // плагин загружен, но отключен
                                ...
                                document.getElementById('PluginEnabledImg').setAttribute("src", "app/crypto_pro/sources/red_dot.png");
